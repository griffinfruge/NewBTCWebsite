...
class PayPalExpressGateway extends PayPalFunctions {
  ...
  public function retrieve_token() {
    ...
    $items        = array();
    $description   = '';
    //echo nl2br(print_r($cart_details, true));
    foreach( $cart_details as $c ) {
      $option_name = '';
      
      if(!empty($c['item_number']['options'])) {
        foreach($c['item_number']['options'] as $price_id) {
          $option_name .= edd_get_price_option_name( $c['id'], $price_id );
        }
        
      }
      $c['name'] = !empty($option_name) ? $c['name'] . ' - ' . $option_name : $c['name'];

      $description .= html_entity_decode( $c['name'], ENT_COMPAT, 'UTF-8' ) . ', ';
      $item_data   = array(
        'name'     => html_entity_decode( $c['name'], ENT_COMPAT, 'UTF-8' ),
        'amount'   => $c['subtotal'],
        'number'   => $c['item_number']['id'],
        'quantity' => $c['quantity'],
        'tax'      => $c['tax']
      );
    ...
    if( ! empty( $this->_purchase_data['fees'] ) ) {
      foreach( $this->_purchase_data['fees'] as $fee ) {
        $description .= html_entity_decode( $fee['label'], ENT_COMPAT, 'UTF-8' ) . ', ';
        $item_data   = array(
    ...
    $paypal_data = array(
      ...
      'PAYMENTREQUEST_0_TAXAMT' => number_format($tax, 2, '.', ''),
