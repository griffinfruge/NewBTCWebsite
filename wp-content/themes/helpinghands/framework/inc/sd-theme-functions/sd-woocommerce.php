...
if ( !function_exists( 'sd_prods_nr' ) ) {
	function sd_prods_nr() {
		...
		$prods_nr = 'return ' . $sd_data['sd_products_nr'] . ';';
...
add_action( 'woocommerce_proceed_to_checkout', 'sd_woo_checkout_button', 10 );

function sd_woo_checkout_button() {
...
/*
// Add wishlist in single product summary
if ( class_exists( 'YITH_WCWL' ) && $sd_data['sd_woo_wishlist'] == 1 ) {
	if ( !function_exists( 'sd_woo_wisglist_summary' ) ) {
		function sd_woo_wisglist_summary() {
			echo '<div class="sd-add-wishlist">' . do_shortcode( '[yith_wcwl_add_to_wishlist]' ) . '</div>';
		}
		add_action( 'woocommerce_single_product_summary', 'sd_woo_wisglist_summary', 29 );
	}
}
*/
// Chance add to cart text if product is in cart
...
/*
// add check mark on items already in cart
if ( !function_exists( 'sd_item_in_cart' ) ) {
function sd_item_in_cart() {
	global $woocommerce;
	...
	foreach( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
			...
			if( get_the_ID() == $_product->id ) {
				echo '<span class="sd-loading-cart"><i class="fa fa-check"></i></span>';
			}
	}
	
}
	add_filter( 'woocommerce_before_shop_loop_item_title', 'sd_item_in_cart' );
}
*/
// Alter price html
