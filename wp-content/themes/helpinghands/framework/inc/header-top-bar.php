	...
	<div class="container">
		...
		<?php if ( $sd_data['sd_social_icons'] == '1' ) : ?>	
			...
			<?php foreach ( $sd_data['sd_social_icons_data'] as $font_class => $url ) {
					if ( $url ) { ?>
						<a class="sd-header-<?php if ( $font_class == 'email' ) { echo 'email'; } else { echo esc_attr( $font_class ); } ?>" href="<?php if ( $font_class == 'email' ) { echo 'mailto: ' . esc_attr( $url ) ; } else if ( $font_class == 'phone' ) { echo '#'; } else { echo esc_url( $url ); } ?>" title="<?php echo esc_attr( $font_class ); ?>" target="<?php if ( $font_class == 'phone' ) { echo '_self'; } else { echo '_blank'; } ?>" rel="nofollow"><i class="sd-link-trans fa fa-<?php if ( $font_class == 'email' ) { echo 'envelope-o'; } else { echo esc_attr( $font_class ); } ?>"></i> <?php if ( $font_class == 'email' || $font_class == 'phone' ) { echo $url; } ?> </a>
			<?php   }
				  } 
			?>
			</div>
